...
class ReplayCache:
    ...
    def _parse_replay(self, replay_path: Path, match_id: int) -> ParsedReplayData:
        """Parse all needed data from replay file in a single pass using v2 API."""
        replay_str = str(replay_path)

        # Single-pass parsing with all collectors enabled
        parser = Parser(replay_str)
        logger.info(f"Starting single-pass parse for match {match_id}...")
        ...
        # Extract combat log entries
        combat_log = self._extract_combat_log(result)
        logger.info(f"Extracted {len(combat_log)} combat log entries")

        # Extract entity snapshots, tick-time map, and hero positions
        entity_snapshots, tick_time_map, hero_positions = self._extract_entities(result)
        logger.info(f"Extracted {len(entity_snapshots)} entity snapshots")

        # Parse metadata separately (uses different message type)
    ...
    def _extract_combat_log(self, result) -> List[CombatLogEntry]:
        """Extract combat log entries from parse result."""
        entries = []
        if result.combat_log and result.combat_log.entries:
            for e in result.combat_log.entries:
    ...
    def _extract_entities(
        ...
        """Extract entity snapshots from parse result."""
        snapshots = []
        tick_time_map = []
        hero_positions: Dict[int, Dict[str, Tuple[float, float]]] = {}

        if result.entities and result.entities.snapshots:
    ...
    def _parse_metadata(self, replay_str: str) -> Optional[Dict[str, Any]]:
        """Parse match metadata using v2 API."""
        try:
            parser = Parser(replay_str)
            result = parser.parse(
                messages={
            ...
            if result.messages and result.messages.messages:
                return result.messages.messages[0].data
